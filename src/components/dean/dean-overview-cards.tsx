...
import type { DeanOverviewDto } from "@/services/dean-stats";

interface DeanOverviewCardsProps {
  data?: DeanOverviewDto;
  isLoading?: boolean;
}

export function DeanOverviewCards({
  data,
  isLoading,
}: DeanOverviewCardsProps) {
  if (isLoading) {
    return (
      <>
        {[1, 2, 3, 4].map((i) => (
  ...
  const cards = [
    {
      title: "Giảng viên",
      value: data.totalLecturers.toLocaleString("vi-VN"),
      icon: Users,
      growth: data.lecturersGrowth,
    ...
    {
      title: "Cần chấm điểm",
      value: data.pendingProjects.toLocaleString("vi-VN"),
      icon: ClipboardCheck,
    ...
    {
      title: "Đã chấm điểm",
      value: data.approvedProjects.toLocaleString("vi-VN"),
      icon: CheckCircle2,
      growth: data.approvedGrowth,
    ...
    {
      title: "Hội đồng",
      value: data.totalCouncils.toLocaleString("vi-VN"),
      icon: Users2,
      growth: data.councilsGrowth,
            ...
            <CardContent className="min-w-0">
              ...
              {growth && (
                <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1 flex-wrap">
                  ...
                  <span
                    className={growth.isPositive ? "text-green-600" : "text-red-600"}
                  >
