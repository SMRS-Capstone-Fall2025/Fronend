...
type TaskDetailDialogProps = {
  ...
  readonly memberOptions: ProjectMemberOption[];
  readonly assignSelection: string;
  readonly onAssignSelectionChange: (value: string) => void;
  readonly onAssignMember: () => void;
  readonly isAssigning: boolean;
  readonly isProjectDetailError: boolean;
  readonly isProjectMembersLoading: boolean;
  readonly projectMembersErrorMessage: string;
...
export function TaskDetailDialog({
  ...
  const activeTaskStatus = task?.status ?? null;
  const isKnownTaskStatus =
    activeTaskStatus != null &&
    Object.prototype.hasOwnProperty.call(statusStyles, activeTaskStatus);
  const activeTaskStatusMeta = isKnownTaskStatus
    ? columnMeta[activeTaskStatus as TaskStatus]
    : null;

  const activeTaskDeadlineLabel = formatDateDisplay(task?.deadline);
  const activeTaskStartLabel = formatDateDisplay(task?.startDate);
  const activeTaskMilestoneLabel = task?.milestoneName?.trim() ?? null;
  const activeTaskDescription = task?.description?.trim() ?? null;
  const activeTaskAssigneeId =
    task?.assignedTo?.id != null ? task.assignedTo.id : null;
  const currentAssigneeId =
    activeTaskAssigneeId != null ? String(activeTaskAssigneeId) : null;
                ...
                <div className="space-y-3 flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {activeTaskMilestoneLabel ? (
                        <p className="text-sm text-blue-100">
                          Gắn với mốc {activeTaskMilestoneLabel}
                        </p>
                      ) : null}
                    </div>
                  </div>
                ...
                <>
                  <div className="grid gap-4 md:grid-cols-3">
                    <InfoCard
                      ...
                      value={activeTaskDeadlineLabel ?? "Chưa có hạn"}
                      helper={
                        activeTaskDeadlineLabel ? null : "Chưa được đặt hạn"
                      }
                    />
                    <InfoCard
                      icon={Clock}
                      label="Ngày bắt đầu"
                      value={activeTaskStartLabel ?? "Chưa xác định"}
                      helper={
                        activeTaskStartLabel ? null : "Chưa có lịch bắt đầu"
                      }
                    />
                    <InfoCard
                      icon={Milestone}
                      label="Báo cáo"
                      value={activeTaskMilestoneLabel ?? "Chưa gắn mốc"}
                    />
                  </div>
                        ...
                        <>
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                            ...
                            <Button
                              ...
                              disabled={
                                !assignSelection ||
                                assignSelection === currentAssigneeId ||
                                isAssigning
                              }
                              className="sm:w-auto"
                            >
                          ...
                          <p className="text-xs text-muted-foreground">
                            Người được chọn sẽ nhận thông báo cập nhật ngay lập
                            tức.
                          </p>
                        </>
                      )}
                  ...
                  <div className="rounded-xl border border-border/60 bg-background/80 p-5 shadow-sm">
                    ...
                    <p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">
                      {activeTaskDescription || "Chưa có mô tả"}
                    </p>
                  </div>
                  ...
                  <div className="rounded-xl border border-border/60 bg-background/90 p-5 shadow-sm">
                    ...
                    <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                      ...
                      <p>
                        Trạng thái hiện tại:
                        <span className="ml-1 font-medium text-foreground">
                          {activeTaskStatusMeta?.title ?? "Chưa xác định"}
                        </span>
                      </p>
                    </div>
                  </div>
